...
import {
  listAdViewsForSession,
  listCampaigns,
  type AdViewWithCampaign,
} from "@/lib/db";
import { getAnonSession } from "@/lib/session";
import type { Campaign } from "@/data/types";

export const dynamic = "force-dynamic";

export default async function MyImpactPage() {
  const sessionId = await getAnonSession();
  const { env } = await getCloudflareContext({ async: true });
  const adViews = sessionId
    ? await listAdViewsForSession(env.DB, sessionId, { limit: 100 })
    : [];
  ...
  const campaigns =
    totalAds > 0 ? await listCampaigns(env.DB, { limit: 100 }) : [];
  const campaignById = new Map(campaigns.map((c) => [c.id, c]));

  const byCampaign = adViews.reduce<
  ...
  const campaignBreakdown = Object.entries(byCampaign)
    ...
    .filter((item): item is { campaign: Campaign; count: number; amount: number } =>
    ...
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="p-4 rounded-[10px] bg-[#FEF3E8] border border-[#F28C28]/20 text-center">
          ...
          <p className="text-xs font-medium text-[#C96F1A] uppercase tracking-wider mt-1">
            Raised
          </p>
        </div>
        <div className="p-4 rounded-[10px] bg-[#EBF3FE] border border-[#2B7DE9]/20 text-center">
          ...
          <p className="text-xs font-medium text-[#2B7DE9] uppercase tracking-wider mt-1">
            Ads Watched
          </p>
        </div>
        <div className="p-4 rounded-[10px] bg-[#E6F5EB] border border-[#34A853]/20 text-center">
          ...
          <p className="text-xs font-medium text-[#34A853] uppercase tracking-wider mt-1">
            Campaigns
          </p>
        </div>
      </div>
      ...
      {totalAds === 0 ? (
        ...
        <>
          ...
          <RecentActivity adViews={adViews.slice(0, 10)} />
        </>
      )}
    </div>
  );
...
function RecentActivity({ adViews }: { adViews: AdViewWithCampaign[] }) {
  return (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-[#1A1D21]">
              Watched &ldquo;
              <span className="font-medium">{view.adTitle}</span>&rdquo;
            </p>
            <p className="text-xs text-[#8C939E]">{view.campaignTitle}</p>
          </div>
